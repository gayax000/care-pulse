...
export default function FeedbackModal({ isOpen, onClose, onSave, editingFb }) {
  ...
  const validate = () => {
    ...
    if (!formData.doctorOrService.trim()) errs.doctorOrService = 'Doctor or service is required';
    if (!formData.comments.trim()) errs.comments = 'Comments are required';
    setErrors(errs);
...
<input className="form-control" placeholder="Amal Silva" value={formData.patientName} onChange={e => setFormData({...formData, patientName: e.target.value})} />
            {errors.patientName && <div className="error-text">{errors.patientName}</div>}
</div>
<div className="form-group">
<label>Doctor / Department Consulted</label>
<input className="form-control" placeholder="Dr. Kasun Perera (Cardiology)" value={formData.doctorOrService} onChange={e => setFormData({...formData, doctorOrService: e.target.value})} />
...
<div className="form-group">
<label>Patient Experience & Comments</label>
<textarea rows="3" className="form-control" placeholder="Doctor was very attentive and treatment was effective..." value={formData.comments} onChange={e => setFormData({...formData, comments: e.target.value})} />
            {errors.comments && <div className="error-text">{errors.comments}</div>}
</div>
<div className="form-group">
<label>Follow-up Symptom Inquiry (Optional)</label>
<input className="form-control" placeholder="Any mild side effects or recovery questions" value={formData.symptomsInquiry} onChange={e => setFormData({...formData, symptomsInquiry: e.target.value})} />
</div>
